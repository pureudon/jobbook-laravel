...
<div>
    <a href="{{ route('invoice.create') }}" class="editor">New</a>
</div>
...
@if($datatables_checkboxselect===true)
<form id="checkboxselect" action="{{route('invoice.groupchangeform')}}" method="GET">
...
@endif
    ...
    <table class="display table table-bordered" id="invoice-table" style="width:100%">
        <thead>
            <tr>
                @if($datatables_checkboxselect===true)
                <th>id</th>
...
@if($datatables_columnsearch===true)
            $('#invoice-table tfoot th').each( function () {
                var title = $('#invoice-table thead th').eq( $(this).index() ).text();
...
@endif
            var oTable = $('#invoice-table').DataTable( {
                ...
                columns: [
                    @if($datatables_checkboxselect===true)
                    { data: '{{ $db_invoice_id }}', name: '{{ $db_invoice_id }}', searchable: false, orderable: false },
                    @endif 
...
@endif
                ...
                {
                    var r = $('#invoice-table tfoot tr');
                    r.find('th').each(function(){
                        $(this).css('padding', 8);
                    });
                    $('#invoice-table thead').append(r);
...
@if($datatables_inputsearch===true)
            ...
            var state = oTable.state.loaded();
            @if($datatables_checkboxselect==true)
            var colnameposition = {{ $columns->where($db_column_fieldname,$db_invoice_name)->first()->$db_column_position + 1 }};
            @else
            var colnameposition = {{ $columns->where($db_column_fieldname,$db_invoice_name)->first()->$db_column_position }};
            @endif
            ...
            $('#name').on( 'keyup click', function () {
                ...
                @endif
                $('#invoice-table').DataTable().column( colnameposition ).search(
...
@if($datatables_selectsearch===true)
            ...
            var state = oTable.state.loaded();
            @if($datatables_checkboxselect==true)
            var columncategoryposition = {{ $columns->where($db_column_fieldname,$db_invoice_category)->first()->$db_column_position + 1 }};
            @else
            var columncategoryposition = {{ $columns->where($db_column_fieldname,$db_invoice_category)->first()->$db_column_position }};
            @endif
            ...
            $('#category').on( 'change', function () {
                ...
                @endif
                if ( $('#category').val()=='any' ) {
                    $('#invoice-table').DataTable().column( columncategoryposition ).search('',true,false,true).draw();
                }
                else{
                    $('#invoice-table').DataTable().column( columncategoryposition ).search($('#category').val().trim().replaceAll(" ", "|"),true,false,true).draw();
                }
            } );
            ...
            var state = oTable.state.loaded();
            @if($datatables_checkboxselect==true)
            var columnfontcolorposition = {{ $columns->where($db_column_fieldname,$db_invoice_fontcolor)->first()->$db_column_position + 1 }};
            @else
            var columnfontcolorposition = {{ $columns->where($db_column_fieldname,$db_invoice_fontcolor)->first()->$db_column_position }};
            @endif
            ...
            $('#fontcolor').on( 'change', function () {
                ...
                if ( $('#fontcolor').val()=='any' ) {
                    $('#invoice-table').DataTable().column( columnfontcolorposition ).search('',true,false,true).draw();
                }
                else{
                    $('#invoice-table').DataTable().column( columnfontcolorposition ).search($('#fontcolor').val().trim().replaceAll(" ", "|"),true,false,true).draw();
                }
            } );
@endif
...
@if($datatables_clearfilter===true)
            $('#clearfilter').on( 'click', function () {
                ...
                $('#invoice-table').DataTable().columns('').search('');
                // oTable.draw();
                ...
                var colnameposition = {{ $columns->where($db_column_fieldname,$db_invoice_name)->first()->$db_column_position }};
                @endif
                $('#invoice-table').DataTable().column( colnameposition ).search($('#name').val(),true,false,true);

                // category (reset to deafult value)
                $('#category').val('');
                @if($datatables_checkboxselect==true)
                var colnameposition = {{ $columns->where($db_column_fieldname,$db_invoice_category)->first()->$db_column_position + 1 }};
                @else
                var colnameposition = {{ $columns->where($db_column_fieldname,$db_invoice_category)->first()->$db_column_position }};
                @endif
                $('#invoice-table').DataTable().column( colnameposition ).search($('#category').val(),true,false,true);
                ...
                var columnfontcolorposition = {{ $columns->where($db_column_fieldname,$db_invoice_fontcolor)->first()->$db_column_position }};
                @endif
                $('#invoice-table').DataTable().column( columnfontcolorposition ).search('',true,false,true);
